<template>
  <view class="preview-container" v-if="currentInfo">
    ...
    <!-- 底部弹窗 -->
    <uni-popup ref="popup" type="bottom" is-mask-click :safe-area="false">
...
<script setup>
import { ref } from "vue";
import { getDownloadWall, setScore, getSinglePicDetail } from "@/api/base.js";
...
onLoad(async ({ id, type }) => {
  ...
  if (type === "share") {
    ...
    storageClassList = res.data;
  } else {
    storageClassList = uni.getStorageSync("storageClassList") || [];
  }
...
onShareAppMessage(() => {
  return {
    title: `我的壁纸-预览`,
    path: `/pages/preview/preview?id=${classifyId.value}&type=share`,
